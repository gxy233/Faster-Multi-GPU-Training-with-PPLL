...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_c=3, num_classes=1000,
        ...
        dupths=[2,2,2,1,1,1,0,0,0,0,0,0]
        # dupths=[3,3,3,2,2,2,1,1,1,0,0,0]
        # dupths=[4,4,4,3,3,3,2,2,2,1,1,1]
    ...
    def get_block_count(self,aux_depth, max_depth):
        return max(0,max_depth - aux_depth+1)
    
    
    def backward_process(self,aux_classifier, x, target):
        """
        子进程计算损失并执行反向传播，只计算梯度，不更新参数。
        """
        # 计算损失
...
class VisionTransformer_base_front(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_c=3, num_classes=1000,
                 embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.0, qkv_bias=True,
                 qk_scale=None, representation_size=None, distilled=False, drop_ratio=0.,
                 attn_drop_ratio=0., drop_path_ratio=0., embed_layer=PatchEmbed, norm_layer=None,
        ...
        Args:
            ...
            norm_layer: (nn.Module): normalization layer
        """
        super(VisionTransformer_base_front, self).__init__()
        self.num_classes = num_classes
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models
        self.num_tokens = 2 if distilled else 1
        norm_layer = norm_layer or partial(nn.LayerNorm, eps=1e-6)
...
class VisionTransformer_base_back(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_c=3, num_classes=1000,
                 embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.0, qkv_bias=True,
                 qk_scale=None, representation_size=None, distilled=False, drop_ratio=0.,
                 attn_drop_ratio=0., drop_path_ratio=0., embed_layer=PatchEmbed, norm_layer=None,
        ...
        Args:
            ...
            norm_layer: (nn.Module): normalization layer
        """
        super(VisionTransformer_base_back, self).__init__()
        self.num_classes = num_classes
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models
        self.num_tokens = 2 if distilled else 1
        norm_layer = norm_layer or partial(nn.LayerNorm, eps=1e-6)
...
class VisionTransformer_base_mid(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_c=3, num_classes=1000,
                 embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.0, qkv_bias=True,
                 qk_scale=None, representation_size=None, distilled=False, drop_ratio=0.,
                 attn_drop_ratio=0., drop_path_ratio=0., embed_layer=PatchEmbed, norm_layer=None,
        ...
        Args:
            ...
            norm_layer: (nn.Module): normalization layer
        """
        super(VisionTransformer_base_mid, self).__init__()
        self.num_classes = num_classes
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models
        self.num_tokens = 2 if distilled else 1
        norm_layer = norm_layer or partial(nn.LayerNorm, eps=1e-6)
...
def vit_base_patch16_224(img_size,patch_size,num_classes: int = 1000):
    """
...
def vit_base_patch16_224_base(img_size,patch_size,num_classes: int = 1000):
    """
    ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    weights ported from official Google JAX impl:
    链接: https://pan.baidu.com/s/1zqb08naP0RPqqfSXfkB2EA  密码: eu9f
    """
    model = VisionTransformer_base(img_size=img_size,
                              patch_size=patch_size,
                              embed_dim=768,
                              depth=12,
                              num_heads=12,
...
def vit_base_patch16_224_base_front(img_size,patch_size,depth,num_classes: int = 1000):
    """
    ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    weights ported from official Google JAX impl:
    链接: https://pan.baidu.com/s/1zqb08naP0RPqqfSXfkB2EA  密码: eu9f
    """
    model = VisionTransformer_base_front(img_size=img_size,
                              patch_size=patch_size,
                              embed_dim=768,
                              depth=depth,
                              num_heads=12,
...
def vit_base_patch16_224_base_back(img_size,patch_size,depth,num_classes: int = 1000):
    """
    ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    weights ported from official Google JAX impl:
    链接: https://pan.baidu.com/s/1zqb08naP0RPqqfSXfkB2EA  密码: eu9f
    """
    model = VisionTransformer_base_back(img_size=img_size,
                              patch_size=patch_size,
                              embed_dim=768,
                              depth=depth,
                              num_heads=12,
...
def vit_base_patch16_224_base_mid(img_size,patch_size,depth,num_classes: int = 1000):
    model = VisionTransformer_base_mid(img_size=img_size,
                              patch_size=patch_size,
                              embed_dim=768,
                              depth=depth,
                              num_heads=12,
