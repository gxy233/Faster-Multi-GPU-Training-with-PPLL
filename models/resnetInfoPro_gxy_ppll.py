...
class InfoProResNet_basic(nn.Module):
    ...
    def __init__(self, block, arch, partname, local_module_num, infopro_config, batch_size, seg_para, image_size=32,
                 inplanes=None, balanced_memory=False, dataset=None, class_num=10, useema=True,
        ...
        if self.partname=='back':
            for item in self.infopro_config[:-1]:
                ...
                self.LB.append(copy.deepcopy(selflayer[f'layer{mo}'][la]))
                self.EMA_Net.append(copy.deepcopy(selflayer[f'layer{mo}'][la]))
    ...
    def _initialize_auxiliary_nets(self, aux_net_config, local_loss_mode, aux_net_widen, aux_net_feature_dim):
        ...
        for index in self.dec_indexs:
            module_index,layer_index=index[0],index[1]
    ...
    def forward_features(self, img, target=None,ixx_1=5, ixy_1=0.5,
        ...
        if self.training:
            ...
            for i in range(len(self.Encoder_Net)):
                ...
                # countf+=forward_end_time-forward_start_time
                ...
                ### ema更新，除了最后一层ema，剩下的层都是用指数移动平均更新
                if i<len(self.Encoder_Net)-1:
                    cur_layer = self.Encoder_Net[i+1]
                ...
                # countb+=backward_end_time-backward_start_time
        
            # print(f'forward time: {countf}    backward time:{countb}')
            return x
...
def main_k2():
    ...
    epoches=400
    # cfg='configs/k2_resnet32_gxy_ppll_k16.yaml'
    cfg='configs/k2_resnet32_gxy_ppll_k8.yaml'
...
# def train(train_loader, front, back, optimizef, epoch, writer):
    ...
    for i, (x, target) in enumerate(tqdm(train_loader)):
        ...
        # print(f'[forward+backward] 用时{(current_time1-current_time).total_seconds() * 1000 } ')
        
        # frontout1=frontout.detach().cpu()
        # target1=target.detach().cpu()
        # x1=x.detach().cpu()
        ...
        # print(f'一个step 用时{(current_time4-current_time).total_seconds() * 1000 }')
...
def train_one_epoch_mid(mid, device, optimizer, in_cache, out_cache):
    ...
    while True:
        ...
        if not in_cache.empty():
            ...
            out_cache.put((pred.detach().cpu(), labels.detach().cpu(), oriimg.detach().cpu()))
...
def train_one_epoch_back(back, optimizer, device, epoch, in_cache,rd_train):
    rd={}
    back.train()
    ...
    rd[epoch] = {'train_loss': losses.value, 'train_accuracy': top1.value}
    
    
      # 加载已有的 JSON 文件内容（如果存在）
    if os.path.exists(rd_train):
        with open(rd_train, "r") as f:
            try:
    ...
    # 更新数据
    existing_data.update(rd)

    # 保存更新后的数据
    with open(rd_train, "w") as f:
...
@torch.no_grad()
def evaluate_back(model, device, epoch, evacache,rd_eva):
    rd={}
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    ...
    # 记录每个epoch对应的accuracy
    rd[epoch] = {'eva_loss': losses.value, 'eva_accuracy': top1.value}

    
      # 加载已有的 JSON 文件内容（如果存在）
    if os.path.exists(rd_eva):
        with open(rd_eva, "r") as f:
            try:
                existing_data = json.load(f)
            except json.JSONDecodeError:
                existing_data = {}
...
#### 初始化front模型
    ...
    front= resnet32_front(infopro_config=infopro_config_f, local_module_num=local_module_num, partname='front', seg_para=seg_paraf, inplanes=f_inplanes, batch_size=1024, image_size=32,
                   dataset=dataset, class_num=10, wide_list=wide_list,
                    dropout_rate=0,
                   aux_net_config=aux_net_config, local_loss_mode=local_loss_mode,
                   aux_net_widen=1, aux_net_feature_dim=128, device=devicef)
...
def train_back(cfg_data, cache, evacache):
    ...
    weight_decay=cfg_data['common']['weight_decay']
    use_coslr=cfg_data['common']['cos_lr']
    rd_train,rd_eva=cfg_data['common']['rd_train'],cfg_data['common']['rd_eva']
    dataset=cfg_data['common']['dataset']
    
    
    # 清空文件内容
    with open(rd_train, 'w') as f:
        f.write('{}')  # 写入空的 JSON 对象，清空文件内容
...
def training(cfg=None):
    ...
    if k == 2:
        ...
        # 创建进程来并行训练
        process_front = mp.Process(target=train_front, args=(train_loader, val_loader, cfg_data, cache, evacache))
        process_back = mp.Process(target=train_back, args=(cfg_data, cache, evacache))
    ...
    # 网络由front，mid，back组成
        ...
        # 创建队列用于进程间通信
        startcache = mp.Queue()
        mid_cache = [mp.Queue() for _ in range(k - 2)]  # 为每个mid创建独立的队列
        startevacache = mp.Queue()
        midevacache = [mp.Queue() for _ in range(k - 2)]
        ...
        # 创建进程来并行训练
        process_front = mp.Process(target=train_front, args=(train_loader, val_loader, cfg_data, startcache, startevacache))
        process_mid_pool = []
        ...
        for i in range(k - 2):
            ...
            if i == 0:
                process_mid =mp.Process(target=train_mid, args=(f'part{i+1}', cfg_data, startcache, mid_cache[i], startevacache, midevacache[i]))
                process_mid_pool.append(process_mid)
            else:
                process_mid = mp.Process(target=train_mid, args=(f'part{i+1}', cfg_data, mid_cache[i-1], mid_cache[i], midevacache[i-1], midevacache[i]))
                process_mid_pool.append(process_mid)

        process_back = mp.Process(target=train_back, args=(cfg_data, mid_cache[-1], midevacache[-1]))
        # process_rd = multiprocessing.Process(target=log_gpu_memory, args=(rd_txt,))
        # process_rd.daemon = True  # 设置为守护进程
...
# def train(train_loader, front, back, optimizef, epoch, writer):
    ...
    for i, (x, target) in enumerate(tqdm(train_loader)):
        ...
        print(f"target.cuda() 用时: {(end_time - start_time) * 1000:.2f} ms")
        ...
        print(f'[数据加载] 用时{(current_time0-current_time).total_seconds() * 1000 } ')
        
        frontout = front.forward_features(img=x, target=target, ixx_1=5, ixy_1=0.5, ixx_2=0, ixy_2=0)
        
        current_time1 = datetime.now()
        print(f'[front forward backward] 用时{(current_time1-current_time0).total_seconds() * 1000 } ')
        
        # frontout=frontout.to('cuda:1')
        # target=target.to('cuda:1')
        # oriimg=oriimg.to('cuda:1')
        
        # current_time15 = datetime.now()
        ...
        current_time3 = datetime.now()
        print(f'[update] 用时{(current_time3-current_time2).total_seconds() * 1000 } ')
