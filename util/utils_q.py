...
def train_one_epoch_front(model, optimizer, data_loader, device, cache, inc_input=False):
    ...
    for step, data in enumerate(data_loader):
        ...
        if inc_input:
            ...
            cache.append((pred.detach().cpu().numpy(),labels.detach().cpu().numpy()))
...
def train_one_epoch_mid(model, optimizer, device, in_cache, out_cache):
    ...
    while True:
        ...
        # if not in_cache.empty():
            ...
            optimizer.step()
            optimizer.zero_grad()
            out_cache.append((pred.detach().cpu().numpy(),labels.detach().cpu().numpy()))
...
def evaluate_front(model, data_loader, device, evacache, inc_input=False):
    ...
    for step, data in enumerate(data_loader):
        ...
        evacache.append((pred.detach().cpu().numpy(),labels.detach().cpu().numpy()))
...
def evaluate_mid(model, device, in_evacache, out_evacache):
    ...
    while True:
        ...
        # if not in_evacache.empty():
            ...
            #
            out_evacache.append((pred.detach().cpu().numpy(),labels.detach().cpu().numpy()))
